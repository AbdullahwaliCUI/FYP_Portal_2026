...
export default async function EvaluatorDashboard() {
    ...
    // 1. Get Evaluator ID and Profile
    const { data: { user } } = await supabase.auth.getUser()
    const { data: appUser } = await supabase.from('app_users').select('id, full_name').eq('auth_user_id', user!.id).single()
    const { data: evaluatorProfile } = await supabase.from('evaluators').select('id').eq('app_user_id', appUser!.id).single()

    if (!evaluatorProfile) {
        return (
            <div className="p-8">
                <h1 className="text-2xl font-bold">Access Denied</h1>
                <p>You do not have an active Evaluator profile. Please contact Admin.</p>
            </div>
        )
    }
    ...
    const { data: projectsRaw } = await supabase
      ...
      *,
      ...
      marks(*)
    `)
        .eq('evaluator_id', evaluatorProfile.id)
...
function ProjectCard({ project }: { project: any }) {
    ...
    const getMark = (comp: string) => project.marks.find((m: any) => m.component === comp && m.by_role === 'evaluator')
...
function MarkForm({ label, projectId, semester, component, maxScore, existingMark, isPF }: any) {
    // Note: Reusing MarkForm concept but submitting to Evaluator Action
    return (
        <form action={submitEvaluatorMark} className="border p-3 rounded bg-slate-50">
